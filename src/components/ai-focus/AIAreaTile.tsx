    ...
    <div className="bg-gradient-to-br from-white to-green-50/30 rounded-xl border border-green-100 shadow-lg relative overflow-hidden h-full flex flex-col transform transition-all duration-300 hover:-translate-y-1 hover:shadow-xl group">
      ...
      <div className="p-10 flex flex-col h-full relative z-10">
        <div className="flex items-center gap-4 mb-6">
          <div className="p-3.5 rounded-lg bg-gradient-to-br from-green-50 to-green-100 text-green-600 shadow-sm transform transition-transform group-hover:scale-110 duration-300">
            <Icon size={32} className="transform transition-transform group-hover:-rotate-12" />
          </div>
        ...
        <div className="space-y-4 mt-auto">
          {items.map((item, i) => (
            <div key={i} className="flex items-center gap-3 group/item">
              <div className="w-2 h-2 rounded-full bg-green-400 transform transition-transform group-hover/item:scale-150"></div>
              <span className="text-gray-700 text-base font-medium group-hover/item:text-green-600 transition-colors">
                {item}
              </span>
            </div>
          ))}
